    ...
    <Page>
        <div class="warehouse-create space-y-4">
            ...
            <div class="warehouse-form border border-blue-500 flex flex-col">
                ...
                <div v-show="isOpenEdit" class="p-6 warehouse-create-form gap-6">
                    <a-form
                        :model="formState"
                        layout="vertical"
                    ...
                    >
                        ...
                        <a-form-item v-bind="validateInfos.phone">
                            ...
                            <a-input id="phone" v-model:value="formState.phone" v-only-numbers type="text" maxlength="10" required></a-input>
                        </a-form-item>
...
const routes = ref([
    ...
    },
    {
        ...
        breadcrumbName: "Chi tiết kho hàng",
    },
]);
    ...
    validate()
        .then(async (res) => {
            const response = await axios.post(
                `/api/warehouses/${router.currentRoute.value.params.id}`,
                {
...
const handleCancel = () => {
    ...
    for (const [key, value] of Object.entries(dataSource.value.data.item)) {
        if (key == "manager_id") {
            formState.value.manager_id = value;
        } else if (key == "name") {
            formState.value.storage_name = value;
        } else if (key == "code") {
            formState.value.storage_code = value;
        } else if (key == "email") {
...
const queryData = (params) => {
    return axios.get(`/api/warehouses/${router.currentRoute.value.params.id}`);
};
