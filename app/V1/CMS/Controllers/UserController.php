...
class UserController extends Controller
{
    ...
    public function index(Request $request): ResourceCollection
    {
        ...
        return $this->responseIndex(UserResource::collection($data));
    }
    ...
    public function update(UpdateRequest $request, $id): JsonResponse
    {
        ...
        if ($id == Auth::id()) {
            return $this->responseUpdateSuccess('', ['item' => new ProfileResource($data)]);
        }
     ...
     * @throws Throwable
     */
    public function syncPermission(SyncPermissionRequest $request, $id): JsonResponse
    {
        try {
            DB::beginTransaction();
            $input = $request->validated();
            $data = $this->model->syncPermissions($id, $input);
            DB::commit();
        } catch (\Exception $exception) {
            DB::rollBack();
            $response = GAK_ERROR::handle($exception, 'users');
